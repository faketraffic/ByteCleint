...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? ByteClient.colorManager.getColorWithAlpha(ByteClient.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : ByteClient.colorManager.getColorWithAlpha(ByteClient.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        drawString(this.getName() + " " + Formatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf((this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) ByteClientGui.getClickGui().getTextOffset(), -1);
    }

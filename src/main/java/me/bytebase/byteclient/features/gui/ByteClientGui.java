package me.bytebase.byteclient.features.gui;
...
import me.bytebase.byteclient.features.gui.items.buttons.ModuleButton;
...
public class ByteClientGui extends Screen {
    private static ByteClientGui byteclientGui;
    private static ByteClientGui INSTANCE;

    static {
        INSTANCE = new ByteClientGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public ByteClientGui() {
        super(Text.literal("ByteClient"));
        setInstance();
        load();
    }

    public static ByteClientGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new ByteClientGui();
        }
        return INSTANCE;
    ...
    public static ByteClientGui getClickGui() {
        return ByteClientGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : ByteClient.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    ByteClient.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
